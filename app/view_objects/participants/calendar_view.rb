...
module Participants
  class CalendarView < ::CalendarView
    def zoom_link_display(_reservation)
      '<em><small>Der Zoom-Link wird per E-Mail verschickt.</small></em>'
    end

    def attendance_list(reservation)
      %{<br>offene vor Ort Plätze: <b>#{reservation.onsite_space_remaining}</b> <small>(besetzt: #{reservation.onsite_attendance_count}; Limit: #{reservation.onsite_limit})</small>}
    end
