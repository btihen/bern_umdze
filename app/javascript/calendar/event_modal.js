  ...
  if ($modalButtons.length > 0) {
    $modalButtons.forEach(function ($el) {
      $el.addEventListener('click', function () {
        ...
        var dataHeaderHtml = this.getAttribute("data-modal-header");
        var dataBodyHtml   = this.getAttribute("data-modal-body");

        // inject html from the date-modal attribute
        document.getElementById(`${target}-header`).innerHTML = dataHeaderHtml
        document.getElementById(`${target}-body`).innerHTML = dataBodyHtml
